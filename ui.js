...
import { initFileActions } from "./popups/file-actions/file-actions.js?v=26-07-07-5";
import { openPersonPopup } from "./popups/person-popup.js?v=26-07-07-5";
...
function renderReadonlyCard(applicantId) {
  ...
  const editPerson = `<button type="button" class="small secondary icon-btn readonly-person-edit-btn" data-edit-readonly-person="${esc(applicant.person_id)}" title="Edit applicant person" aria-label="Edit applicant person">✎</button>`;
  ...
  box.querySelector("[data-edit-readonly-person]")?.addEventListener("click", event => editReadonlyPerson(event.currentTarget.dataset.editReadonlyPerson));
}

async function editReadonlyPerson(personId) {
  if (!personId) return toast("Select an applicant first.");
  await openPersonPopup({ state: stateRef, commit: commitRef, personId, fromApplicant: true, onSaved: () => document.dispatchEvent(new CustomEvent("sir:data-changed")) });
}
