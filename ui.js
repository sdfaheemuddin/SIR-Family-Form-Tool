...
import { buildReadonly, formatAadhaar, onlyDigits } from "./core.js?v=26-07-08-19";
...
function whatsappLink(phone) { return /^\d{10}$/.test(phone) ? `<div class="readonly-actions whatsapp-actions"><a class="button-link secondary whatsapp-link" href="https://wa.me/91${esc(phone)}" target="_blank" rel="noopener">WhatsApp</a></div>` : ""; }
...
function deletePerson(personId) { const used = stateRef.applicants.some(applicant => [applicant.person_id, applicant.mapper_person_id, applicant.father_person_id, applicant.mother_person_id, applicant.spouse_person_id].filter(Boolean).includes(personId)); if (used) return toast("Cannot delete: person is used in applicants."); if (!confirm(`Delete person ${personName(personId)}?`)) return; stateRef.people = stateRef.people.filter(person => person.person_id !== personId); commitRef(); refreshScreen(); notifyDataChanged({ reason: "person-deleted" }); }
function deleteApplicant(applicantId) { const applicant = stateRef.applicants.find(row => row.applicant_id === applicantId); if (!confirm(`Delete applicant ${personName(applicant?.person_id)}?`)) return; stateRef.applicants = stateRef.applicants.filter(row => row.applicant_id !== applicantId); commitRef(); refreshScreen(); notifyDataChanged({ reason: "applicant-deleted" }); }
...
function markComplete() { const id = $("#readonlyApplicantSelect").value; const applicant = stateRef.applicants.find(row => row.applicant_id === id); if (!applicant) return; applicant.status_completed = true; commitRef(); renderDatabaseTables(); selectNext(id); notifyDataChanged({ reason: "status-changed", selectedApplicantId: $("#readonlyApplicantSelect").value }); }
