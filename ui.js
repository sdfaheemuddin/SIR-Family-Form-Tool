...
function renderApplicants() {
  ...
  wrap.querySelectorAll("[data-toggle-status]").forEach(input => input.addEventListener("change", () => {
...
function deletePerson(personId) {
  ...
  if (used) return toast("Cannot delete: person is used in applicants.");
...
function selectNext(currentId = "") {
  const rows = pendingApplicants();
  const select = $("#readonlyApplicantSelect");
  if (!rows.length) { select.dataset.selected = ""; renderReadonlyPicker(); toast("All applicants completed."); return; }
...
export function initUI(state, commit) {
  ...
  initZoom();
  addVersion();
  initFileActions({ state: stateRef, commit: commitRef, renderAll: refreshScreen, toast });
  $("#clearDataBtn").addEventListener("click", clearAll);
  $("#readonlyApplicantSelect").addEventListener("change", event => { event.target.dataset.selected = event.target.value; renderReadonlyCard(event.target.value); syncReadonlyButtons(); });
  ...
  document.addEventListener("sir:data-changed", refreshScreen);
  refreshScreen();
}
